...
AND athletes_teams.team_id = (teams.id WHERE teams.team = "Kenya");
...
SELECT medals.medal 
FROM medals 
WHERE (medals.id = event_performances.medal_id 
AND event_performances.athlete_id = (athletes.id 
where athletes.athlete = "%Greg Louganis%") 
ORDER BY games.year 
WHERE games)id = event_performances.game_id );
